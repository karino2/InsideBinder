...
Androidはこの機能をフル活用していて、例えば 本編 第2巻 5章の内容の裏側では、ActivityThreadの内部クラスであるApplicaitonThreadのプロキシをBinderで送ったり、
BundleやActivityRecordなどもBinderで送ったりされていました。
そういった意味ではBinderというのはAndroidにとって重要な技術と言えます。

一方で本文書は、本編の書籍よりも読者に要求する前提知識が多くなっています。
そこで最初に@<hd>{sec81}で、あまり前提知識が無い読者を想定して、Androidという物を理解するのに必要なBinderの最低限の所を説明する事から始める事にしました。

あまりこの分野に馴染みのない人がどこまで本文書を読むのかは、@<hd>{sec81}を読んで、自身で判断してもらいたい、と思っています。
@<hd>{sec81}の内容さえ理解しておけば、Androidの他の部分のコードを読むのには差し支えありません。
本文書が難しいと思ったら@<hd>{sec81}だけ読む、という読み方もアリだと思います。

ですが、もし分散オブジェクトにある程度の理解があるなら、本文書を最後まで読めば、Binderについての全てを理解する事が出来るでしょう。
//}

=={sec81} 最低限のBinder基礎知識 - 分散オブジェクトを知らない人向け

Binderは、使うのは簡単だけれど実現する実装が複雑、という物です。
使い方を覚えるのは比較的簡単で、それだけ分かっていれば、AndroidのうちBinder以外の部分を理解するには十分です。

そこで本章では、難解な実現方法は気にせずに、利用者の視点に限定して必要最低限な話をしていきたいと思います。
...
そこで本章では、なるべく専門用語などを出さずに、上記の事を説明してみたいと思います。
逆に分散オブジェクトに詳しくてBinderの詳細に興味がある人は、本章の内容はあとでより詳細に扱うので、流し読みで十分です。
@<chapref>{main_intro}から真面目に読み始めていただけたら、と思います。
...
== Binderの通常の使われ方 - getSystemService()メソッド
...
まず本編のコラムでもたまに言及される、分散オブジェクトの代表としてはCOMが挙げられます。
...
== IBinderとXX.Stub.asInterfaceとXX.Stub.Proxy
