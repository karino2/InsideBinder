= AIDLからの自動生成 - Javaによるシステムサービスの実装
...
かなりの部分はアプリ開発者でも行う事が出来ます。(詳細は@<hd>{AIDLとコンパイルとサービスの実装手順}を参照)

//}

== Javaのシステムサービスを支える基本的なクラス
...
== AIDLのコンパイルで生成されるクラス - StubとStub.ProxyとStub.asInterface()
...
== 実際のACCOUNT_SERVICE呼び出しにみる、システムサービス呼び出しの例
